...
**⏱️ זמן משוער:** 3-4 ימים  
**🎯 מטרה:** מניעת דאבל בוקינג עם Hold זמני  
**🛠️ כלי:** Cursor (Python + FastAPI + Redis)
...
**מה צריך לשמור:**
- `cabins` - העברת נתונים מ-Google Sheets ל-DB
- `bookings` - שמירת כל הזמנה אחרי `/book`
- `customers` - שמירת לקוחות חדשים
- `pricing_rules` - קריאת חוקים מ-DB (במקום hardcoded)
- `transactions` - אחרי שלב 5 (תשלומים)

📋 **לפרטים מלאים:** ראה `docs/PROJECT_STATUS.md` ו-`docs/INTEGRATION_GUIDE.md`
